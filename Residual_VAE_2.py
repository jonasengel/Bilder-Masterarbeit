...
beta_max = 1
wu_epochs = epochs//1      #Anzahl der warmup Epochen

k1 = 3
k2 = 3
k3 = 3

model = 'Residual_VAE_2'
...
checkpoint_filepath = '/home/itoengeljon/Masterarbeit/Software/' + model + '/' + model + '_weights.h5'
...
warmup_callback = LambdaCallback(on_epoch_begin=lambda epoch, log : warmup(epoch))
...
if load_weights is True:
    vae.load_weights('/home/itoengeljon/Masterarbeit/Software/' + model + '/' + model + '_weights.h5')
else:
    pass
...
class Predictor():
    ...
    def tsne(self):
        ...
        latent_var = latent_var.reshape(len(os.listdir(img_path + '/Test/test')), latent_dim)           #Umformung von latent Var Liste aus predict zu Array
...
P.predictor()
P.img_algebra([28, 27, 24])
P.feature_scatter(method='tsne')
'''for i in range(10):
    print(np.mean(P.predictor()[i]))'''